...
#+SEQ_TODO: TODO INPROGRESS DELE-FOLLOW-UP | DONE CANCELED 
#+AUTHOR: Jason Chen
...
** TODOLIST
*** TODO Check reviewboard reporting feature
*** TODO Update bugzilla
*** TODO Think about how we can actively handle installation failure cases, but not just stay on the restore utility. If so, there is no need to wait for the installation failure and then we provide a workaround. Read some paper from lisa (large linux system administration)
- introduce git / svn design
*** TODO follow up C++ db connection issue - CM db connection module improvement to reduce dependency to system db availability
*** TODO share atmosconfig and new db pylib module in the installation refactoring process
*** DELE-FOLLOW-UP Hold installation/serviceability join meeting to discuss unify disk mgmt seperate from configuration
...
** Atmos 1.2.7 cycle
*** TODO Setup MaaS similar testbed and try to reproduce random subtenant creation issues
*** DONE fix pgdbsetup dup record - wait Yubo response
	 CLOSED: [2010-04-20 Tue 11:23]
*** DONE eBay migration DC planning
	 CLOSED: [2010-04-20 Tue 11:23]
------------------------------- spliter -------------------------------
first draft 
...
** Atmos 1.3.2 cycle
*** TODO Follow up on corbis installation
Hi Sukwoo, Ming,
...
*** INPROGRESS Mongrel session clear cronjob patch
*** TODO Improve installation error log on CM side. bug #11860
*** Review mgmt DB related bugs and make planning
**** DELE-FOLLOW-UP pgdbcheck related bugs should be fixed in 1.3.2
...
In slave node:
1. remoteListenThread (remoteListen_receive_events) get one SYNC event from sl_event table (from local or remote? should be remote) and put to worker queue. Here reload SYNC event large group maxsize is 100 by code default.
2. remoteWorkerThread (remoteWorkerThread_main) receives SYNC event from remoteListenThread and insert event to local sl_event table
3. remoteWorkerThread (sync_event) invoke remoteHelpThread to (where is sl_log_status?) read from sl_log_x table for the real data
...
4. remoteHelpThread will get the data from local db and apply to the local table, then wait in another loop for WorkerGroupData
5. remoteWorkerThread (sync_event) will set seqid, seq_last_value and setsync table for seqno and snapshot
