...
** TODOLIST
*** TODO Read REST code from Yong and give feedbacks
*** INPROGRESS [#A] decouple genprofile for better organise and rereuse code, consider ip change and reconfiguration together.
*** INPROGRESS review board review process workflow
...
|-- local_listen.c -> Wait for local event and scan for event
|-- remote_listen.c  (remoteListen_receive_event()) -> Receive event from db tables and pass them to worker thread
|-- remote_worker.c  (sync_event()) -> Process event provided by remote listen thread
